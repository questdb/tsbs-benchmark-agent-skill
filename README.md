...
When co-locating, pin the client and server to disjoint CPU sets (`--cpuset-cpus` on the QuestDB container, `taskset` on the loader) so ILP and QWP see identical, non-competing budgets. The loaders are light, so give the server almost all the cores and the client just a couple (e.g. 30 and 2 on a 32-core box), and keep the server's core count the same co-located as networked, or the network run is really measuring a bigger server rather than the network.

At very high cardinality, `--qwp-tags-as-varchar` sends tags as VARCHAR instead of QWP SYMBOL, so no per-frame symbol dictionary is shipped; the server still stores them as SYMBOL when the table is pre-created that way. It costs more bytes per row at low cardinality but avoids the dictionary growth that inflates QWP frames as the number of distinct series climbs.
